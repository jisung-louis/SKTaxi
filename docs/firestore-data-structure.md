...
| 경로 | 설명 | 주요 필드/하위 구조 |
| --- | --- | --- |
| `parties/{partyId}` | 택시 모집 카드 | `leaderId`, `departure{name,lat,lng}`, `destination{name,lat,lng}`, `departureTime`(ISO), `maxMembers`, `members[]`, `tags[]`, `detail`, `status ('open'|'closed'|'arrived')`, `createdAt`, `updatedAt`, `settlement.{status,perPersonAmount,members.{uid}.{settled,settledAt}}` |
...
| `chatRooms/{chatRoomId}/messages/{messageId}` | 공개방 메시지 | `text`, `senderId`, `senderName`, `type ('text'|'image'|'system')`, `readBy[]`, `direction`, `source`, (Minecraft 연동 필드) `minecraftUuid`, `appUserDisplayName`, `createdAt` |
...
| `appNotices/{noticeId}` | 운영 공지(앱 내 카드) | `title`, `content`, `category ('update'|'service'|'event'|'policy')`, `priority ('urgent'|'normal'|'info')`, `imageUrl`, `actionUrl`, `publishedAt`, `updatedAt` |
...
| `cafeteriaMenus/{weekId}` | 주차별 학식 | `weekStart`, `weekEnd`, `rollNoodles[]`, `theBab[]`, `fryRice[]`, `createdAt`, `updatedAt` |
...
| `reports/{reportId}` | 신고 접수 | `targetType ('post'|'comment'|'chat_message'|'profile')`, `targetId`, `targetAuthorId`, `category`, `reasonText`, `screenshotUrls[]`, `reporterId`, `status`, `createdAt`, `updatedAt` |
