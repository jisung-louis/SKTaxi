...
└── MainNavigator (로그인 + 프로필 + 권한 완료)
    └── Bottom Tab Navigator
        ...
        └── CommunityTab
            ...
            ├── BoardWriteScreen
            ├── BoardEditScreen
            ├── ChatListScreen (내부 라우트 유지)
            └── ChatDetailScreen (채팅방 내부)
```

> 탭 바는 각 탭의 최상위 화면에서만 표시되고, 하위 Stack 화면에서는 자동으로 숨겨집니다.
> 현재 `CommunityTab`은 제품 로드맵에 맞춰 기존 게시판 화면으로 우선 연결되어 있으며, 커뮤니티 상단 탭 UI는 후속 단계에서 교체 예정이다.
...
firebase-cloud-functions/            Firebase Cloud Functions (Node 22)
...
**관련 파일:** `src/features/auth/screens/`
...
**관련 파일:** `src/features/home/screens/HomeScreen.tsx`
...
- `src/features/chat/data/minecraftChatBridge.ts`에서 메시지 전송 함수 제공
- `src/features/minecraft/`에서 계정 UUID 조회 및 등록
- RTDB 구독은 `src/features/chat/screens/ChatDetailScreen.tsx`에서 직접 처리

---

### 4.7 알림 & 개인 설정

**관련 파일:** `src/features/user/screens/NotificationScreen.tsx`, `src/features/settings/screens/SettingScreen.tsx`, `src/features/user/screens/NotificationSettingsScreen.tsx`
...
- 회원 탈퇴는 즉시 처리한다.
- `members` row는 soft delete tombstone으로 유지하고, 개인정보는 스크럽한다.
- 같은 Firebase UID는 재가입에 재사용하지 않는다. 탈퇴한 동일 UID의 `POST /v1/members`는 `409 WITHDRAWN_MEMBER_REJOIN_NOT_ALLOWED`를 반환한다.
- Phase 10 이전 운영 DB를 올릴 때는 앱 기동 전에 `members.status`를 수동 마이그레이션으로 채워야 한다. 자세한 절차는 `docs/deployment-guide.md`를 따른다.
- 탈퇴 시 `linked_accounts`, `user_notifications`, `fcm_tokens`, `user_timetables`는 삭제한다.
- 게시글/댓글/공지댓글은 본문을 보존하고 작성자 표시만 `탈퇴한 사용자`로 익명화한다.
- 택시 파티 리더가 탈퇴하면 active party는 `WITHDRAWED`로 종료된다.
...
- 앱 시작 시 `src/app/bootstrap/versionCheck.ts`에서 현재 버전 비교 → `forceUpdate: true`이면 업데이트를 강제하고, `showButton=true`면 스토어 이동 버튼을 노출
...
**관련 파일:** `firebase-cloud-functions/src/index.ts`
...
- **앱 → 게임**: 클라이언트가 RTDB에 직접 메시지 작성 (`src/features/chat/data/minecraftChatBridge.ts`)
- RTDB 구독(서버 상태, 접속자 수)은 `ChatDetailScreen`에서 직접 처리
- `src/features/minecraft/`에는 계정/실시간 상태 관련 로직이 모여 있다
