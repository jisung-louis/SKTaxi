...
- Phase 8 verifier blocker 수정으로 shared moderation의 actual source of truth는 이제 pure contract/helper인 `src/shared/lib/moderation/{contracts,index}.ts` 다. Firebase Firestore moderation 구현의 actual source of truth는 `src/shared/lib/firebase/moderationRepository.ts` 로 이동했고, `src/features/board/data/composition/boardModerationRuntime.ts`, `src/features/chat/data/composition/chatModerationRuntime.ts` 가 허용된 composition 경계에서 repository를 생성한다. `src/shared/lib/moderation/MockModerationRepository.ts` 는 이번 턴에 keep-as-is 로 남긴다. root `src/repositories/interfaces/IModerationRepository.ts`, `src/repositories/firestore/FirestoreModerationRepository.ts`, `src/repositories/mock/MockModerationRepository.ts` 는 shim-only re-export만 유지한다.
...
- Phase 8 blocker cleanup: shared subscription contract 수렴으로 `src/features/user/data/repositories/IUserRepository.ts`, `src/features/chat/data/repositories/IChatRepository.ts`, `src/features/taxi/data/repositories/IPartyRepository.ts` 의 로컬 `SubscriptionCallbacks`/`Unsubscribe` 정의를 제거했다. active repository contract와 notice/minecraft subscription consumer는 이제 shared contract를 직접 사용하고, `src/features/{user,chat,taxi}/index.ts` 는 generic subscription contract를 다시 export하지 않는다.
- Phase 8 lint hard gate cleanup으로 active source tree lint error 7건을 제거했다. 대상은 `src/features/taxi/components/JoinRequestModal.tsx`, `src/shared/ui/{CommentInput,ForegroundNotification,HashTagText,TimePicker}.tsx` 이며, 현재 `./node_modules/.bin/eslint src/app src/features src/shared --ext .ts,.tsx` 는 `0 errors` 이고 `npm test -- --runInBand` 도 통과했다.
...
- warning baseline cleanup으로 `src/app/navigation/MainNavigator.tsx`, `src/features/home/screens/HomeScreen.tsx`, `src/features/minecraft/screens/MinecraftDetailScreen.tsx`, test helper와 route helper 수정 파일의 대상 eslint 경고를 줄였다. 현재 `./node_modules/.bin/eslint src/app src/features src/shared --ext .ts,.tsx` 기준 결과는 0 errors 이고 warning baseline은 남아 있다.
- 검증 결과:
