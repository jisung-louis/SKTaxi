...
- non-auth runtime orchestration은 `src/app/navigation/AppNavigationRuntime.tsx` 로 분리되어 `RootNavigator` 바깥에서 동작한다.
...
- join request modal, foreground notification, FCM orchestration은 `src/app/navigation/AppNavigationRuntime.tsx` 에서 계속 legacy `src/navigations/hooks/*` 를 재사용한다. RootNavigator는 얇아졌지만 이 runtime host 자체는 이후 feature 분리 단계에서 더 줄여야 한다.
