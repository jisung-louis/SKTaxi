...
export const useChatScreenPresenter = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPartyTitle(getPartyTitle(party));
      } catch {
        setPartyTitle('채팅');
      }
    })();
  ...
  const handleSend = useCallback(async () => {
    if (!message.trim() || !partyId || !currentUser?.uid) return;
  ...
  }, [currentUser?.uid, message, partyId, partyRepository]);
  ...
  const handleAcceptJoin = useCallback(async (requestId: string, requesterId: string) => {
    ...
    try {
      await acceptJoinRequest({
        ...
        requestId,
        requesterId,
        requesterName: displayNameMap[requesterId] || '익명',
      });
  ...
  }, [currentUser?.uid, notificationActionRepository, partyId, partyRepository]);
  ...
  const handleArrivalSubmit = useCallback(async () => {
    ...
    try {
      ...
      // 정산 데이터 구성
      const initialSettlementStatus = buildInitialSettlementStatus(
        selectedMembers,
        currentParty?.leaderId,
      );
      const settlementMembers: SettlementData['members'] =
        buildSettlementMembers(selectedMembers, currentParty?.leaderId);
  ...
  }, [currentUser?.uid, displayNameMap, partyId, partyRepository, settlementStatus]);
  ...
  }, [currentParty, currentUser?.uid, handleArrivalConfirm, handlePartyEnd, isLeader, loadUserAccount, partyId, partyRepository]);
  ...
  }, [currentUser?.displayName, currentUser?.uid, displayNameMap, navigation, notificationRepository, partyId, partyRepository]);
  ...
  const handleLeaderDeleteParty = useCallback(async (isPartyArrived = false) => {
    if (!partyId || !currentUser?.uid || !isLeader || deleteHandledRef.current) return;
