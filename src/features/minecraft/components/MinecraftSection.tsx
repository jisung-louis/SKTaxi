...
export const MinecraftSection: React.FC = () => {
  ...
  const handleRegisterMinecraft = async () => {
    ...
    try {
      await registerAccount(mcNickname.trim(), mcEdition);
      setMcNickname('');
      Alert.alert('등록 완료', '화이트리스트에 등록이 완료되었어요.');
    } catch (nextError: any) {
      Alert.alert('오류', nextError?.message || '등록 중 오류가 발생했습니다.');
    }
  };
    ...
    <View style={styles.section}>
      ...
      {loading ? (
        <View style={styles.minecraftRegisteredCard}>
          ...
          <Text style={styles.minecraftInfoText}>계정 정보를 불러오는 중이에요.</Text>
        </View>
      ...
      {minecraftAccounts.length < 4 && (
        <View style={styles.minecraftForm}>
          ...
          {error ? <Text style={styles.minecraftErrorText}>{error}</Text> : null}
          <Button
            title="등록하기"
